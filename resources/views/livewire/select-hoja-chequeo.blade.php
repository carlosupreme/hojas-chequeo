<div class="relative flex flex-col h-[calc(100vh-1rem)] sm:h-[calc(100vh-2rem)] space-y-4 font-sans">
    <div wire:loading.delay wire:target="selectHojaChequeo, selectHojaEjecucion, loadMore"
        ...
        <div
            class="flex flex-col items-center gap-3 p-4 rounded-2xl bg-white dark:bg-gray-800 shadow-xl border border-gray-100 dark:border-gray-700 transform scale-100 animate-in fade-in zoom-in-95 duration-200">

            <div class="relative w-12 h-12">
                <div class="absolute inset-0 rounded-full border-[3px] border-gray-100 dark:border-gray-700"></div>
                <div
                    class="absolute inset-0 rounded-full border-[3px] border-t-blue-600 border-r-transparent border-b-transparent border-l-transparent animate-spin">
                </div>
            </div>
    ...
    <div class="flex-none px-4 sm:px-6 z-20">
        <div
            ...
            <div class="relative w-full md:w-64 shrink-0">
                ...
                <input wire:model.live.debounce.300ms="search" type="search" placeholder="Filtrar equipos..."
            ...
            <div class="flex-1 flex items-center gap-1 overflow-x-auto no-scrollbar md:mask-linear-fade">
                <button wire:click="$set('activeFilter', null)" class="px-4 py-1.5 rounded-lg text-xs font-bold transition-colors whitespace-nowrap
    ...
    : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800'
                ...
                @foreach($areas as $area)
                            <button wire:click="toggleFilter('{{ $area }}')" class="px-4 py-1.5 rounded-lg text-xs font-bold transition-colors whitespace-nowrap uppercase
                                    {{ $activeFilter === $area
                    ? 'bg-gray-900 text-white dark:bg-white dark:text-gray-900'
                    : 'text-gray-500 hover:bg-gray-100 dark:text-gray-400 dark:hover:bg-gray-800'
                                    }}">
    ...
    <div class="flex-1 overflow-hidden">
        {{--
        Layout Logic:
        - Mobile/Tablet (< lg): Flex Row with Horizontal Scroll + Snap. - Desktop (lg+): Grid 3 columns. --}} <div
            class="h-full w-full px-4 sm:px-6 pb-4
