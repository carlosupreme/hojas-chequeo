@props(['status', 'equipo', 'date', 'version', 'capacidad' => null])
...
<div
    class="group relative bg-white dark:bg-gray-800 rounded-xl p-3 shadow-sm hover:shadow-md transition-all duration-200 cursor-pointer border border-gray-100 dark:border-gray-700 {{ $config['border'] }}">
    <div class="flex gap-3">

        {{-- 1. Avatar (Fixed Size) --}}
        <div class="shrink-0">
            <div
                class="h-16 w-16 rounded-xl bg-gray-50 dark:bg-gray-700 overflow-hidden border border-gray-100 dark:border-gray-600 flex items-center justify-center">
                @if ($equipo->foto)
                    <img src="{{ asset('storage/' . $equipo->foto) }}" class="h-full w-full object-cover" alt="">
                @else
                    <svg class="h-6 w-6 text-gray-300 dark:text-gray-500" fill="none" viewBox="0 0 24 24"
                         stroke="currentColor">
        ...
        <div class="flex-1 min-w-0 flex flex-col justify-between">
            ...
            <div>
                <h4 class="text-sm font-black text-gray-900 dark:text-white truncate leading-tight">
                    {{ $equipo->tag }}
                    @if($capacidad)
                        <span class="ml-1 text-[10px] font-semibold text-primary-600 dark:text-primary-400">{{ $capacidad }}</span>
                    @endif
                </h4>
            ...
            <div class="flex items-end justify-between mt-2">
                {{-- Status Badge --}}
                <span
                    class="inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-bold uppercase tracking-wide {{ $config['badge_bg'] }}">
                    @if($status === 'pending')
