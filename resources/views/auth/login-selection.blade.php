...
<body class="min-h-screen bg-[var(--bg-base)] text-[var(--text-primary)] antialiased transition-colors duration-300"
    ...
    <!-- Navigation -->
    <nav
        class="fixed top-0 left-0 right-0 z-40 bg-[var(--bg-nav)] backdrop-blur-xl border-b border-[var(--border-color)] transition-colors duration-300">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex items-center justify-between h-16">
                <div class="flex items-center gap-4">
                    <img :src="theme === 'dark' || (theme === 'system' && window.matchMedia('(prefers-color-scheme: dark)').matches) ? '/dark.png' : '/logo.png'"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative" @click.away="themeMenuOpen = false">
                        <button @click="themeMenuOpen = !themeMenuOpen"
                            ...
                            <svg x-show="theme === 'light'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                            ...
                            <svg x-show="theme === 'system'" xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <!-- Dropdown -->
                        <div x-show="themeMenuOpen" x-transition:enter="transition ease-out duration-100"
                            ...
                            <div class="py-1">
                                <button @click="theme = 'light'; themeMenuOpen = false"
                                    class="group flex w-full items-center px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-hover-glass)] hover:text-[var(--text-primary)]">
                                    <svg class="mr-3 h-5 w-5 text-[var(--text-muted)] group-hover:text-[var(--text-primary)]"
                                ...
                                <button @click="theme = 'dark'; themeMenuOpen = false"
                                    class="group flex w-full items-center px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-hover-glass)] hover:text-[var(--text-primary)]">
                                    <svg class="mr-3 h-5 w-5 text-[var(--text-muted)] group-hover:text-[var(--text-primary)]"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
                                ...
                                <button @click="theme = 'system'; themeMenuOpen = false"
                                    class="group flex w-full items-center px-4 py-2 text-sm text-[var(--text-secondary)] hover:bg-[var(--bg-hover-glass)] hover:text-[var(--text-primary)]">
                                    <svg class="mr-3 h-5 w-5 text-[var(--text-muted)] group-hover:text-[var(--text-primary)]"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor">
    ...
    <!-- Main Content -->
    <main class="min-h-screen pt-28 pb-12 px-4" x-data="{
              selectedUser: null,
              password: '',
              showModal: false,
              error: null,
        ...
        <div class="max-w-5xl mx-auto">
            ...
            <div class="max-w-md mx-auto mb-10 animate-fade-in-up" style="animation-delay: 0.1s">
                <div class="relative">
                    ...
                    <input type="text" x-model="searchQuery" placeholder="Buscar operador..."
                    ...
                    <button x-show="searchQuery" x-cloak @click="searchQuery = ''"
                ...
                <template x-for="(operador, index) in filteredOperadores" :key="operador.id">
                    <button @click="selectUser(operador, index)"
                        ...
                        <div class="relative mb-4">
                            <div class="w-20 h-20 sm:w-24 sm:h-24 rounded-xl bg-gradient-to-br flex items-center justify-center text-white font-bold text-2xl sm:text-3xl shadow-lg group-hover:shadow-xl transition-all duration-300"
                                :class="getAvatarColor(index)">
                                <span x-text="getInitials(operador.name)"></span>
                            </div>
                            <!-- Online indicator -->
                            <div
                                class="absolute -bottom-1 -right-1 w-5 h-5 bg-green-500 rounded-full border-4 border-[var(--bg-base)] opacity-0 group-hover:opacity-100 transition-opacity">
                            </div>
                        </div>
            ...
            <!-- Empty State -->
            <div x-show="filteredOperadores.length === 0" x-cloak class="text-center py-16">
                <svg class="mx-auto h-16 w-16 text-[var(--text-tertiary)] mb-4" xmlns="http://www.w3.org/2000/svg"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div x-show="showModal" x-transition:enter="transition ease-out duration-300"
                ...
                <div class="p-8">
                    ...
                    <div class="flex flex-col items-center mb-8">
                        <div class="w-24 h-24 rounded-xl bg-linear-to-br flex items-center justify-center text-white font-bold text-3xl shadow-lg mb-4 pulse-glow"
