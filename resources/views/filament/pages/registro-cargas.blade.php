    ...
    <div class="space-y-4 pb-8">
            ...
            <div class="overflow-x-auto -mx-4 px-4">
                <div class="flex gap-3 pb-2" style="width: max-content">
                    @foreach($tombolas as $tombola)
                                    ...
                                    <button wire:click="selectEquipo({{ $tombola->id }})" wire:key="tom-{{ $tombola->id }}"
                        ...
                        : 'border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900 hover:border-gray-300 dark:hover:border-gray-600' }}">
                                        ...
                                        <div
                                            class="font-bold text-sm {{ $isSelected ? 'text-blue-700 dark:text-blue-300' : 'text-gray-800 dark:text-gray-200' }} truncate">
                                            {{ $tombola->tag }}
                                        </div>
        ...
        @if($equipo)
            ...
            <div class="grid grid-cols-3 gap-3">
                <div
                    class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 p-3 text-center">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                @empty
                    <div class="px-4 py-8 text-center">
                        <svg class="w-10 h-10 text-gray-200 dark:text-gray-700 mx-auto mb-2" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
            ...
            <div class="bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 overflow-hidden">
                ...
                <div class="px-4 py-3 border-b border-gray-100 dark:border-gray-800">
                    <div class="flex flex-wrap items-start justify-between gap-3 mb-3">
                        <div>
                            <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300">Historial de cargas</h3>
                            <p class="text-xs text-gray-400 mt-0.5">{{ $historial->count() }}
                                {{ Str::plural('registro', $historial->count()) }}</p>
                        </div>
                    </div>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
                                <th
                                    class="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                    Fecha / Hora</th>
                                <th
                                    class="px-4 py-2.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">
                                    Tombola</th>
                        ...
                        <tbody class="divide-y divide-gray-50 dark:divide-gray-800/60">
                            @forelse($historial as $carga)
                                <tr class="hover:bg-gray-50/60 dark:hover:bg-gray-800/30 transition-colors">
                                    <td
                                        class="px-4 py-2.5 font-mono text-xs text-gray-600 dark:text-gray-400 whitespace-nowrap">
                                        {{ $carga->registrado_en->format('d/m/Y') }}
                                        <span
                                            class="text-gray-400 dark:text-gray-500 ml-1">{{ $carga->registrado_en->format('H:i') }}</span>
                                    </td>
                                    <td class="px-4 py-2.5">
                                        <span class="font-semibold text-gray-800 dark:text-gray-200 font-mono text-xs">
                                            {{ $carga->equipo?->tag ?? '—' }}
                                        </span>
                                    </td>
